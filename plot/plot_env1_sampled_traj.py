...
def plot_full_trajectory():
    ...
    obstacles = np.array([
       [-1.0, 0.0, 0.3],  # x=-1.0, y=0.0, radius=0.3
    ...
    for traj_set in sampled_trajs:  
        for traj in traj_set:  
            ax.plot(traj[:, 0], traj[:, 1],linewidth=10.0 ,linestyle='-', color='red', alpha=0.3)
    ...
    with open("optimal_traj_log.txt", "r") as f_opt:
        ...
        for line in f_opt:
            ...
            if line.startswith("Frame") or "TimeStep" in line:
                if current_traj:
                    first_point = np.array(current_traj[0]) 
                    if len(first_point) == 3:
                        optimal_points.append(first_point)
                    current_traj = []
            else:
                try:
    ...
    if len(optimal_points) > 0:
        ax.plot(optimal_points[:, 0], optimal_points[:, 1], color='cyan', linestyle='-', linewidth=10.0, label="Robot Path")
